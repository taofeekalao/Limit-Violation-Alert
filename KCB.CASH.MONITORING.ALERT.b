...
* <Rating>953</Rating>
*-----------------------------------------------------------------------------
    SUBROUTINE KCB.CASH.MONITORING.ALERT
*
*   *------------------------------------------------------------------------------------------
    $INSERT I_COMMON
    $INSERT I_EQUATE
...
*REPORT.TIME
    LOCATE "REPORT.TIME" IN R.PARAM<KCB.SYS.PARAM.NAME,1> SETTING IP.POS THEN
        REPORT.TIME = R.PARAM<KCB.SYS.PARAM.VALUE.1,IP.POS>
        CHANGE ':' TO '' IN REPORT.TIME
        CHANGE '.' TO '' IN REPORT.TIME
    END
* 180000
    IF LEN(REPORT.TIME) < 6 THEN
        REPORT.TIME = FMT(REPORT.TIME,'L2%7')
        CHANGE '.' TO '' IN REPORT.TIME
    END

    JBASE.TIME = OCONV(TIME(),"MTS")[1,8]
    CHANGE ':' TO '' IN JBASE.TIME

    IF JBASE.TIME < REPORT.TIME  THEN
        PROCESS = 0
        RETURN
    END ELSE
        PROCESS = 1
...
********
    ...
    FOR JJ = 1 TO NO.RECS
        ...
        SEL.CMD = "SSELECT ":FN.KCB.L.MSL.CASH.MGMT:" WITH @ID LIKE ":YBRANCH:"..."
...
*    Y.MESSAGE = ALERT.MESSAGE: ' ':Y.MESSAGE
...
*    Y.MSG = "java -cp ./BE.BP/SocketConnector.jar com.SocketServer ":Y.IP:" ":Y.PORT:" '":Y.CONV.OUT:"'"
*    EXECUTE Y.MSG CAPTURING Y.RESPONSE
...
*    Y.RESP.DATA = Y.RESPONSE["<StatusCode>",2,1]
*    Y.RESP.CODE = Y.RESP.DATA[1,1]

*    Y.RESP.DATA = Y.RESPONSE["<StatusDesc>",2,1]
*    Y.RESP.ERR =  FIELD(Y.RESP.DATA,'<',1,1)
